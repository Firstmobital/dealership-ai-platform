...
export function CampaignsModule() {
  ...
  const { templates, fetchApprovedTemplates, loading: templatesLoading } =
  ...
  const selectedMsgs = selectedCampaign ? messages[selectedCampaign.id] ?? [] : [];

  /* --------------------------------------------------------------------
     ANALYTICS (SAFE FALLBACK)
  -------------------------------------------------------------------- */
  const analytics = useMemo(() => {
    const total = selectedCampaign?.total_recipients ?? selectedMsgs.length ?? 0;
